...
tar --extract --file=bash.tar.gz --strip-components=1 --directory=/usr/local/src/bash
rm bash.tar.gz

if [ -d bash-patches ]; then
    apk add --no-cache --virtual .patch-deps patch
    for p in bash-patches/*; do
        patch --directory=/usr/local/src/bash --input="$(readlink -f "$p")" --strip=0
        rm "$p"
    done
    rmdir bash-patches
...
for p in /usr/local/src/tianon-bash-patches/*; do
    patch --directory=/usr/local/src/bash --input="$p" --strip=1
done
